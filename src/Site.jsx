...
import {
  STATS, RACES, CLASSES, RACE_PREVIEWS, CLASS_PREVIEWS,
  WIKI_ITEMS, WIKI_MOBS, WIKI_RECIPES, ITEM_CATS, QUALITY_COLORS, MOB_CATS, SALVAGE_CATS,
  DUNGEONS, DUNGEON_TIERS, ROLE_META,
} from './data.js';
import { G } from './styles.jsx';
import { decodeBuild } from './engine.js';
...
function CommunityPage({ setPage }) {
  ...
  // Decode build code on paste
  useEffect(() => {
    if (!pubCode.trim()) { setDecoded(null); return; }
  ...
  const handlePublish = async () => {
    if (!pubCode.trim() || !pubName.trim() || !pubAuthor.trim()) {
      setPubError("Code, nom et pseudo sont requis."); return;
    }
    if (pubAuthor.trim().length > 20) {
      setPubError("Pseudo max 20 caractères."); return;
    }
    if (!decoded) {
      setPubError("Code de build invalide."); return;
    }
    ...
    try {
      ...
      setPubSuccess(true);
      setPubCode(""); setPubName(""); setPubDesc(""); setDecoded(null);
  ...
  return (
    <div style={{ position: "relative", zIndex: 1, padding: "100px 24px 60px", maxWidth: 1200, margin: "0 auto" }}>
      {/* Header */}
      <div style={{ display: "inline-block", padding: "4px 16px", borderRadius: 4, background: G.teal + "10", border: "1px solid " + G.teal + "20", fontSize: 11, fontWeight: 800, color: G.teal, textTransform: "uppercase", letterSpacing: 2, marginBottom: 12 }}>
        Partage de builds
      </div>
      <h1 style={{ fontSize: 38, fontWeight: 900, color: "#fff", fontFamily: "var(--fd)", margin: "0 0 8px", letterSpacing: 1 }}>
        Communauté
      ...
      <p style={{ fontSize: 16, color: G.muted, margin: "0 0 24px" }}>
        Parcours les builds de la communauté ou partage les tiens.
      </p>

      {/* Actions bar */}
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
        <button onClick={() => setShowPublish(!showPublish)} style={{
          padding: "10px 22px", borderRadius: "var(--radius-md)", border: "2px solid " + G.teal,
          background: showPublish ? G.teal + "20" : G.teal + "10", color: G.teal,
          fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "var(--fb)",
          display: "flex", alignItems: "center", gap: 8,
        }}>
          {showPublish ? "✕ Annuler" : "📤 Publier un build"}
        </button>
        <button onClick={() => setPage("builds")} style={{
          padding: "10px 22px", borderRadius: "var(--radius-md)", border: "2px solid " + G.border,
      ...
      {showPublish && (
        <div style={{
          background: G.card, border: "1px solid " + G.teal + "30", borderRadius: "var(--radius-lg)",
          padding: 24, marginBottom: 24,
        }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", fontFamily: "var(--fd)", marginBottom: 16 }}>
            📤 Publier un build
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            <div>
              <label style={{ fontSize: 12, color: G.muted, fontWeight: 700, display: "block", marginBottom: 4 }}>Code du build *</label>
              <input value={pubCode} onChange={e => setPubCode(e.target.value)} placeholder="Colle ton code ici (depuis Build Creator → Exporter)" style={inp} />
              {pubCode && !decoded && <div style={{ fontSize: 11, color: "#ff6b6b", marginTop: 4 }}>Code invalide</div>}
              {decoded && (
                <div style={{ fontSize: 11, color: G.teal, marginTop: 4, display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <span>· Niv.{decoded.level} P{decoded.prestige}</span>
                </div>
              )}
            </div>
            <div>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <textarea value={pubDesc} onChange={e => setPubDesc(e.target.value)} placeholder="Explique ta stratégie, les forces et faiblesses du build..." rows={3}
                style={{ ...inp, resize: "vertical" }} maxLength={500} />
            </div>
          </div>
          {pubError && <div style={{ color: "#ff6b6b", fontSize: 13, marginTop: 10 }}>{pubError}</div>}
          {pubSuccess && <div style={{ color: G.teal, fontSize: 13, marginTop: 10, fontWeight: 700 }}>✓ Build publié avec succès !</div>}
          <button onClick={handlePublish} disabled={publishing} style={{
            marginTop: 14, padding: "12px 32px", borderRadius: "var(--radius-md)",
            border: "none", background: G.teal, color: "#000", fontWeight: 800, fontSize: 14,
            cursor: publishing ? "wait" : "pointer", fontFamily: "var(--fb)", opacity: publishing ? 0.6 : 1,
          }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        <select value={filterRace} onChange={e => setFilterRace(e.target.value)} style={{ ...sel, maxWidth: 180 }}>
          <option value="">Toutes les races</option>
          {RACES.map(r => <option key={r.id} value={r.id}>{r.emoji} {r.name}</option>)}
        </select>
        <select value={filterClass} onChange={e => setFilterClass(e.target.value)} style={{ ...sel, maxWidth: 180 }}>
          <option value="">Toutes les classes</option>
          {CLASSES.map(c => <option key={c.id} value={c.id}>{c.emoji} {c.name}</option>)}
        </select>
        <select value={filterRole} onChange={e => setFilterRole(e.target.value)} style={{ ...sel, maxWidth: 180 }}>
          <option value="">Tous les rôles</option>
          {allRoles.map(r => <option key={r} value={r}>{r}</option>)}
        </select>
        {(filterRace || filterClass || filterRole) && (
          <button onClick={() => { setFilterRace(""); setFilterClass(""); setFilterRole(""); }} style={{
            padding: "10px 16px", borderRadius: "var(--radius-md)", border: "1px solid " + G.border,
            background: "transparent", color: G.muted, fontSize: 13, cursor: "pointer", fontFamily: "var(--fb)",
          }}>✕ Reset</button>
        )}
      </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: 60, color: G.muted }}>
          ...
          <div style={{ fontSize: 16 }}>{builds.length === 0 ? "Aucun build trouvé." : "Aucun build ne correspond aux filtres."}</div>
          <div style={{ fontSize: 13, marginTop: 6 }}>{builds.length === 0 ? "Sois le premier à partager le tien !" : "Essaie d'élargir tes critères."}</div>
        </div>
      )}
      ...
      {!loading && !error && filtered.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {filtered.map(b => {
            ...
            return (
              <div key={b.id} style={{
                background: G.card, border: "1px solid " + G.border, borderRadius: "var(--radius-md)",
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: G.border, color: G.muted }}>
                        par {b.author}
                      </span>
                    </div>
                    {b.description && <div style={{ fontSize: 13, color: G.muted, marginBottom: 8, lineHeight: 1.5 }}>{b.description}</div>}
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                      {race && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: race.color + "15", color: race.color, fontWeight: 700 }}>
                        {race.emoji} {race.name}
                      </span>}
                      {cls && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: cls.color + "15", color: cls.color, fontWeight: 700 }}>
                        {cls.emoji} {cls.name}
                      </span>}
                      {cls2 && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: cls2.color + "15", color: cls2.color, fontWeight: 600 }}>
                        + {cls2.name}
                      </span>}
                      {b.role && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: G.purple + "15", color: G.purple, fontWeight: 600 }}>
                        {b.role}
                      </span>}
                      <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: G.border, color: G.muted }}>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                    <button onClick={() => copyCode(b.code, b.id)} style={{
                      padding: "8px 16px", borderRadius: "var(--radius-md)", border: "1px solid " + (isCopied ? G.teal : G.border),
