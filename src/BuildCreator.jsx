...
function DpsTab({computedStats,selectedRace,primaryClass,selectedEvo,selectedAugments}){
  ...
  const tdStyle={padding:"6px 10px",fontSize:11,color:"#ccc",borderBottom:"1px solid #0e1828",verticalAlign:"middle"};
  ...
  return(<div>
    ...
    <div style={{marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:3,background:"#0b1628",borderRadius:8,padding:3,width:"fit-content"}}>
        {views.map(v=>(
          <button key={v.id} onClick={()=>setView(v.id)} style={{padding:"7px 16px",borderRadius:6,border:"none",background:view===v.id?"#1a2d4f":"transparent",color:view===v.id?"#fff":"#555",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:5,transition:"all .15s"}}>
            ...
            {RACES.map((r,ri)=>(
              <tr key={r.id} style={{background:hlRace===r.id?r.color+"08":ri%2===0?"transparent":"#ffffff02"}}>
                <td onClick={()=>setHlRace(hlRace===r.id?null:r.id)}
                  style={{padding:"6px 10px",color:hlRace===r.id?r.color:"#666",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",borderRight:"1px solid var(--brd)",borderBottom:"1px solid #0e1828",display:"flex",alignItems:"center",gap:5,transition:"all .1s"}}>
                ...
                {CLASSES.map(c=>{
                  ...
                  return(
                    <td key={c.id} title={`${r.name} + ${c.name} — ${stars(syn.score)}\n${syn.reason}`}
                      style={{textAlign:"center",padding:"5px 4px",background:hl?sc+"18":"transparent",borderBottom:"1px solid #0e1828",cursor:"default",transition:"background .1s"}}>
