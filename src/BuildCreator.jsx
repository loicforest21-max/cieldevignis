...
function DpsTab({computedStats,selectedRace,primaryClass,selectedEvo,selectedAugments}){
  ...
  const MOD_COLORS={"WansWonder":"#e74c3c","Endgame":"#a55eea","Dexx":"#3498db","Exarkys":"#e67e22","RPGMagic":"#2ed573","CombatGauntlets":"#f368e0","Custom":"#f5a623","ALL":"#888"};
  const QUALITY_COLORS={"Relic":"#f5a623","Legendary":"#e74c3c","Epic":"#3498db","Rare":"#2ed573","Divine":"#9b59b6","Corrupted":"#636e72","Uncommon":"#74b9ff","Common":"#95a5a6","Custom":"#f5a623"};

  const toggleSort=(col)=>{if(sortBy===col)setSortDir(d=>d==="asc"?"desc":"asc");else{setSortBy(col);setSortDir("desc");}};
  const SortIcon=({col})=>sortBy===col?<span style={{color:"#f5a623"}}>{sortDir==="asc"?"▲":"▼"}</span>:<span style={{color:"#333"}}>⇅</span>;

  const filteredWeapons=WEAPONS_REAL
    ...
    .sort((a,b)=>{
      ...
      if(sortBy==="dps"){va=a.avgDmg*a.speed;vb=b.avgDmg*b.speed;}
      else if(sortBy==="avgDmg"){va=a.avgDmg;vb=b.avgDmg;}
      else if(sortBy==="speed"){va=a.speed;vb=b.speed;}
      else{va=a.ilvl;vb=b.ilvl;}
  ...
  const augEffects=[];
  sa.forEach(aug=>{
    if(!aug.dpsImpact)return;const d=aug.dpsImpact;
  ...
  const inp={background:"#12100c",border:"1px solid #2a2218",borderRadius:8,padding:"5px 8px",fontSize:13,fontWeight:700,textAlign:"center",outline:"none"};
  ...
  const thStyle={padding:"7px 10px",textAlign:"left",fontWeight:700,fontSize:10,color:"#7c8db5",textTransform:"uppercase",letterSpacing:0.8,cursor:"pointer",userSelect:"none",whiteSpace:"nowrap",background:"#0b1628",borderBottom:"1px solid var(--brd)"};
  const tdStyle={padding:"6px 10px",fontSize:11,color:"#ccc",borderBottom:"1px solid #1a1610",verticalAlign:"middle"};

  return(<div>
    <h3 style={{margin:"0 0 14px",fontSize:18,color:"#fff",fontFamily:"var(--fd)"}}>⚡ DPS Meter</h3>

    {/* ── ARME ── */}
    <div style={{background:"var(--card)",borderRadius:8,border:"1px solid var(--brd)",marginBottom:10,overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,padding:"12px 14px 8px",flexWrap:"wrap"}}>
        <div style={{fontSize:13,fontWeight:800,color:"#fff",fontFamily:"var(--fd)"}}>🗡️ Arme sélectionnée</div>
        {/* Selected weapon badge */}
        <div style={{display:"flex",alignItems:"center",gap:6,background:"#f5a62312",border:"1px solid #f5a62330",borderRadius:6,padding:"4px 10px"}}>
          <span style={{fontSize:12}}>{weapon.icon}</span>
          <span style={{fontSize:12,fontWeight:700,color:"#f5a623"}}>{weapon.name}</span>
          {weapon.ilvl>0&&<span style={{fontSize:9,color:MOD_COLORS[weapon.mod]||"#888"}}>{weapon.mod} · iLvl {weapon.ilvl}</span>}
          <span style={{fontSize:9,color:dmgType==="magic"?"#a55eea":"#ff9f43"}}>{dmgType==="magic"?"✨":"⚔️"} {baseDmg.toFixed(1)} dmg · {baseSpd}/s</span>
        </div>
        <button onClick={()=>setShowTable(p=>!p)} style={{marginLeft:"auto",background:"transparent",border:"1px solid var(--brd)",borderRadius:5,color:"#555",fontSize:10,padding:"3px 10px",cursor:"pointer"}}>{showTable?"▲ Masquer":"▼ Choisir"}</button>
      </div>

      {showTable&&<div style={{padding:"0 14px 12px"}}>
        {/* Filters */}
        <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
          <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
            {allMods.map(m=><button key={m} onClick={()=>setModFilter(m)} style={{padding:"2px 8px",borderRadius:4,border:"1px solid "+(modFilter===m?(MOD_COLORS[m]||"#f5a623"):"var(--brd)"),background:modFilter===m?(MOD_COLORS[m]||"#f5a623")+"15":"transparent",color:modFilter===m?(MOD_COLORS[m]||"#f5a623"):"#555",fontSize:9,fontWeight:700,cursor:"pointer"}}>{m}</button>)}
          </div>
          <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
            {allTypes.map(t=><button key={t} onClick={()=>setTypeFilter(t)} style={{padding:"2px 8px",borderRadius:4,border:"1px solid "+(typeFilter===t?"#3dd8c5":"var(--brd)"),background:typeFilter===t?"#3dd8c515":"transparent",color:typeFilter===t?"#3dd8c5":"#555",fontSize:9,fontWeight:700,cursor:"pointer"}}>{t}</button>)}
          </div>
        </div>
        ...
        <div style={{overflowX:"auto",borderRadius:6,border:"1px solid var(--brd)"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
            <thead>
              <tr>
                <th onClick={()=>toggleSort("name")} style={thStyle}>Arme <SortIcon col="name"/></th>
                <th style={{...thStyle,cursor:"default"}}>Type</th>
                <th style={{...thStyle,cursor:"default"}}>Mod</th>
                <th onClick={()=>toggleSort("ilvl")} style={thStyle}>iLvl <SortIcon col="ilvl"/></th>
                <th onClick={()=>toggleSort("avgDmg")} style={thStyle}>Dmg moy <SortIcon col="avgDmg"/></th>
                <th onClick={()=>toggleSort("speed")} style={thStyle}>Vitesse <SortIcon col="speed"/></th>
                <th onClick={()=>toggleSort("dps")} style={thStyle}>DPS base <SortIcon col="dps"/></th>
            ...
            <tbody>
              {filteredWeapons.map(w=>{
                ...
                return(
                  <tr key={w.id} onClick={()=>setWeaponId(w.id)} style={{cursor:"pointer",background:sel?"#f5a62308":"transparent",transition:"background .1s"}}
                    onMouseEnter={e=>e.currentTarget.style.background=sel?"#f5a62312":"#ffffff06"}
                    onMouseLeave={e=>e.currentTarget.style.background=sel?"#f5a62308":"transparent"}>
                    <td style={{...tdStyle,fontWeight:sel?700:400,color:sel?"#f5a623":"#ccc"}}>
                      {w.icon} {w.name}
                    </td>
                    <td style={{...tdStyle,color:"#666"}}>{w.type}</td>
                    <td style={{...tdStyle}}><span style={{fontSize:9,color:mc,background:mc+"15",padding:"1px 5px",borderRadius:3,fontWeight:700}}>{w.mod}</span></td>
                    <td style={{...tdStyle,textAlign:"center"}}>{w.ilvl>0?<span style={{fontSize:10,color:qc,fontWeight:700}}>{w.ilvl}</span>:<span style={{color:"#333"}}>—</span>}</td>
                    <td style={{...tdStyle,textAlign:"center",color:"#ff9f43",fontWeight:600}}>{w.avgDmg.toFixed(1)}</td>
                    <td style={{...tdStyle,textAlign:"center",color:"#a29bfe"}}>{w.speed}/s</td>
                    <td style={{...tdStyle,textAlign:"center",color:"#2ed573",fontWeight:700}}>{dpsBase}</td>
                    <td style={{...tdStyle,textAlign:"center"}}>{sel&&<span style={{color:"#f5a623",fontSize:14}}>✓</span>}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        {isCustomW&&<div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginTop:10}}>
    ...
    <div style={{background:"var(--card)",borderRadius:8,padding:14,border:"1px solid var(--brd)",marginBottom:10}}>
      ...
      {enemyMode==="preset"&&<div>
        ...
        <div style={{fontSize:10,color:"#555"}}>PV base: <strong style={{color:"#ff6b6b"}}>{fmtN(preset.hp)}</strong> · Dmg: <strong style={{color:"#e74c3c"}}>{preset.dmg||"?"}</strong> · {preset.mod}</div>
      </div>}
      ...
      <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap",marginTop:8}}>
        ...
        {RPGMOBS_TIERS.map(t=><button key={t.tier} onClick={()=>setRpgTier(t.tier)} style={{padding:"3px 8px",borderRadius:5,border:"2px solid "+(rpgTier===t.tier?t.color:"var(--brd)"),background:rpgTier===t.tier?t.color+"15":"transparent",color:rpgTier===t.tier?t.color:"#555",fontSize:9,fontWeight:700,cursor:"pointer"}} title={t.label}>T{t.tier}</button>)}
        <span style={{fontSize:9,color:tierInfo.color}}>HP×{tierInfo.hpMult} / DMG×{tierInfo.dmgMult}</span>
      </div>
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginTop:8}}>
        ...
        {useEL&&<>
          <input type="number" min={1} max={200} value={enemyLevel} onChange={e=>setEnemyLevel(Math.max(1,Math.min(200,parseInt(e.target.value)||1)))} style={{...inp,width:55,color:"#f5a623"}} title="Niveau"/>
          <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
      ...
      <div style={{marginTop:6,fontSize:9,color:"#3a5068"}}>
        ...
        {useEL&&<span> = {fmtN(baseHP)} × {EL_BASE} × (1 + {EL_PER_LVL} × {enemyLevel})</span>}
      </div>
    </div>
      ...
      {showAugEffects&&<div>
        {augEffects.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(250px,1fr))",gap:3,marginBottom:procs.length>0?8:0}}>
          {augEffects.map((e,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"3px 8px",background:(e.color||"#888")+"08",borderRadius:5,border:"1px solid "+(e.color||"#888")+"12"}}>
            ...
            <span style={{fontSize:9,fontWeight:700,color:e.cond?"#666":e.color}}>{e.effect}</span>
          </div>)}
        </div>}
        {procs.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(270px,1fr))",gap:3}}>
          {procs.map((p,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 8px",background:"#e74c3c08",borderRadius:5,border:"1px solid #e74c3c12"}}>
            <div><span style={{fontSize:10,color:"#ddd",fontWeight:600}}>{p.icon} {p.label}</span>{p.trigger&&<span style={{fontSize:8,color:"#555",marginLeft:4}}>{p.trigger}{p.canCrit?" ·crit":""}</span>}</div>
    ...
    <div style={{marginTop:8,fontSize:9,color:"#3a5068"}}>DPS = baseArme × (1+stat/100) × crit × augMult × atkSpeed{useEL?" · PV = base × RPGMobs × EL("+EL_BASE+"×(1+"+EL_PER_LVL+"×niv))":""}</div>
  </div>);
}
...
function SynergyTab({selectedRace,primaryClass,secondaryClass}){
  ...
  const ROLE_BG={dps_phys:"#ff9f4318",dps_mag:"#a55eea18",tank:"#54a0ff18",support:"#2ed57318",unknown:"#55555518"};
  ...
  const curSyn2=selectedRace&&secondaryClass?getSynergyScore(selectedRace.id,secondaryClass.id):null;
  ...
  const bestSecondary=(selectedRace&&primaryClass)?CLASSES
    .filter(c=>c.id!==primaryClass.id)
    .map(c=>{
      ...
      const compBonus=(c1Dmg==="Physical"&&c.dmg==="Magic")||(c1Dmg==="Magic"&&c.dmg==="Physical")?0.5:0;
  ...
  const ScoreBadge=({score,large})=>(
    <div style={{display:"inline-flex",alignItems:"center",gap:large?6:4,background:(SC[score]||"#555")+"15",border:"1px solid "+(SC[score]||"#555")+"30",borderRadius:6,padding:large?"6px 12px":"3px 8px"}}>
      <span style={{color:SC[score]||"#555",fontSize:large?16:11,lineHeight:1}}>{stars(score)}</span>
      <span style={{color:SC[score]||"#555",fontWeight:700,fontSize:large?12:9}}>{RATING[score]?.split(" ").slice(1).join(" ")}</span>
    </div>
  );

  const RoleTag=({role})=>role&&role!=="unknown"?(
    <span style={{fontSize:9,background:ROLE_BG[role],color:ROLE_C[role],padding:"2px 8px",borderRadius:4,fontWeight:700,letterSpacing:0.3}}>{ROLE_L[role]}</span>
  ):null;

  const views=[{id:"recommend",label:"Mon Build",icon:"🎯"},{id:"top",label:"Top Combos",icon:"🏆"},{id:"matrix",label:"Matrice",icon:"📊"}];

  return(<div>
    {/* Header + view selector */}
    <div style={{marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        <h3 style={{margin:0,fontSize:18,color:"#fff",fontFamily:"var(--fd)"}}>🔗 Synergies Race × Classe Primaire × Secondaire</h3>
      </div>
      <div style={{display:"flex",gap:3,background:"#0b1628",borderRadius:8,padding:3,width:"fit-content"}}>
        {views.map(v=>(
          <button key={v.id} onClick={()=>setView(v.id)} style={{padding:"7px 16px",borderRadius:6,border:"none",background:view===v.id?"#2a2218":"transparent",color:view===v.id?"#fff":"#555",fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:5,transition:"all .15s"}}>
    ...
    {view==="recommend"&&<div>
      {(!selectedRace&&!primaryClass)&&(
        <div style={{padding:32,background:"linear-gradient(135deg,#0e1628,#0b1120)",borderRadius:12,border:"1px solid var(--brd)",textAlign:"center"}}>
          <div style={{fontSize:32,marginBottom:12}}>🔗</div>
          <div style={{fontSize:15,color:"#fff",fontWeight:700,marginBottom:6}}>Configure ton build d'abord</div>
          <div style={{fontSize:12,color:"#555"}}>Sélectionne une race et une classe dans les onglets correspondants.</div>
        </div>
      )}
      ...
      {selectedRace&&primaryClass&&curSyn&&(
        <div style={{marginBottom:20}}>
          {/* Primary combo card */}
          <div style={{background:"linear-gradient(135deg,var(--card),"+( SC[curSyn.score]||"#555")+"08)",borderRadius:12,padding:20,border:"2px solid "+(SC[curSyn.score]||"#555")+"35",marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10,marginBottom:12}}>
              <div>
                <div style={{fontSize:16,fontWeight:800,color:"#fff",marginBottom:6,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <span style={{color:selectedRace.color,background:selectedRace.color+"15",padding:"3px 10px",borderRadius:6}}>{selectedRace.emoji} {selectedRace.name}</span>
                  <span style={{color:"#333"}}>×</span>
                  <span style={{color:primaryClass.color,background:primaryClass.color+"15",padding:"3px 10px",borderRadius:6}}>{primaryClass.emoji} {primaryClass.name}</span>
                  {secondaryClass&&<>
                    <span style={{color:"#333"}}>×</span>
                    <span style={{color:secondaryClass.color,background:secondaryClass.color+"15",padding:"3px 10px",borderRadius:6,opacity:0.8}}>{secondaryClass.emoji} {secondaryClass.name} <span style={{fontSize:9,opacity:0.6}}>(50%)</span></span>
                  </>}
                </div>
            ...
            <p style={{fontSize:12,color:"#aaa",margin:"0 0 10px",lineHeight:1.6}}>{curSyn.reason}</p>
            {curSyn.tags?.length>0&&<div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
              {curSyn.tags.map(t=><span key={t} style={{fontSize:9,background:(SC[curSyn.score]||"#888")+"15",color:SC[curSyn.score]||"#888",padding:"2px 8px",borderRadius:4,fontWeight:700,letterSpacing:0.3}}>{t}</span>)}
            </div>}
          </div>

          {/* Secondary class synergy card */}
          {secondaryClass&&curSyn2&&(
            <div style={{background:"linear-gradient(135deg,var(--card),"+(SC[curSyn2.score]||"#555")+"05)",borderRadius:12,padding:16,border:"1px solid "+(SC[curSyn2.score]||"#555")+"25",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:10,color:"#7c8db5",fontWeight:700,textTransform:"uppercase",letterSpacing:1}}>Synergie secondaire</span>
                  <span style={{color:selectedRace.color,fontSize:12}}>{selectedRace.emoji}</span>
                  <span style={{color:"#333",fontSize:10}}>×</span>
                  <span style={{color:secondaryClass.color,fontSize:12,fontWeight:700}}>{secondaryClass.emoji} {secondaryClass.name}</span>
                </div>
                <ScoreBadge score={curSyn2.score}/>
              </div>
              <p style={{fontSize:11,color:"#888",margin:0,lineHeight:1.5}}>{curSyn2.reason}</p>
              <div style={{marginTop:8,fontSize:10,color:"#666",padding:"6px 10px",background:"#ffffff06",borderRadius:6}}>
                💡 La classe secondaire apporte 50% de ses innates et 50% de ses passifs. Les synergies avec la race restent importantes pour maximiser les bonus.
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:selectedRace&&primaryClass?"1fr 1fr 1fr":"1fr 1fr",gap:14}}>
        {selectedRace&&<div>
          <div style={{fontSize:12,fontWeight:800,color:"#fff",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
            <span style={{color:selectedRace.color}}>{selectedRace.emoji}</span> Meilleures classes pour {selectedRace.name}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {bestForRace.slice(0,8).map(({cls,score,role},i)=>(
              <div key={cls.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"var(--card)",borderRadius:8,border:"1px solid "+(score>=4?(SC[score]||"#555")+"25":"var(--brd)"),transition:"all .15s"}}>
                <span style={{fontSize:11,color:"#555",minWidth:14,textAlign:"right"}}>{i+1}</span>
                <span style={{fontSize:14,color:cls.color}}>{cls.emoji}</span>
                <span style={{fontSize:12,fontWeight:600,color:"#ccc",flex:1}}>{cls.name}</span>
                <RoleTag role={role}/>
                <span style={{fontSize:11,color:SC[score]||"#555",fontWeight:800,letterSpacing:-0.5}}>{stars(score)}</span>
              </div>
            ))}
          </div>
        </div>}
        {primaryClass&&<div>
          <div style={{fontSize:12,fontWeight:800,color:"#fff",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
            <span style={{color:primaryClass.color}}>{primaryClass.emoji}</span> Meilleures races pour {primaryClass.name}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {bestForClass.slice(0,8).map(({race,score,role},i)=>(
              <div key={race.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"var(--card)",borderRadius:8,border:"1px solid "+(score>=4?(SC[score]||"#555")+"25":"var(--brd)")}}>
                <span style={{fontSize:11,color:"#555",minWidth:14,textAlign:"right"}}>{i+1}</span>
                <span style={{fontSize:14,color:race.color}}>{race.emoji}</span>
                <span style={{fontSize:12,fontWeight:600,color:"#ccc",flex:1}}>{race.name}</span>
                <RoleTag role={role}/>
                <span style={{fontSize:11,color:SC[score]||"#555",fontWeight:800,letterSpacing:-0.5}}>{stars(score)}</span>
              </div>
            ))}
        ...
        {selectedRace&&primaryClass&&<div>
          <div style={{fontSize:12,fontWeight:800,color:"#fff",marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
            🛡️ Meilleures classes secondaires
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {bestSecondary.slice(0,8).map(({cls,score,role,adjustedScore},i)=>(
              <div key={cls.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:secondaryClass?.id===cls.id?cls.color+"12":"var(--card)",borderRadius:8,border:"1px solid "+(secondaryClass?.id===cls.id?cls.color+"40":score>=4?(SC[score]||"#555")+"25":"var(--brd)"),transition:"all .15s"}}>
                <span style={{fontSize:11,color:"#555",minWidth:14,textAlign:"right"}}>{i+1}</span>
                <span style={{fontSize:14,color:cls.color}}>{cls.emoji}</span>
                <span style={{fontSize:12,fontWeight:600,color:"#ccc",flex:1}}>{cls.name}</span>
                <RoleTag role={role}/>
                <span style={{fontSize:11,color:SC[score]||"#555",fontWeight:800,letterSpacing:-0.5}}>{stars(score)}</span>
                {cls.passiveScaling&&<span style={{fontSize:8,background:"#a55eea20",color:"#a55eea",padding:"1px 5px",borderRadius:3,fontWeight:700}}>Scale</span>}
              </div>
            ))}
          </div>
          <div style={{marginTop:8,fontSize:10,color:"#555",lineHeight:1.4}}>
            💡 Classement basé sur la synergie Race×Classe + bonus de complémentarité (type de dégâts opposé, passive scaling).
          </div>
        </div>}
      </div>
    </div>}
    ...
    {view==="top"&&<div>
      <div style={{display:"flex",gap:4,marginBottom:14,flexWrap:"wrap"}}>
        {["ALL","dps_phys","dps_mag","tank","support"].map(r=>(
          <button key={r} onClick={()=>setRoleFilter(r)} style={{padding:"4px 12px",borderRadius:6,border:"1px solid "+(roleFilter===r?(r==="ALL"?"#f5a623":ROLE_C[r]):"var(--brd)"),background:roleFilter===r?(r==="ALL"?"#f5a62315":ROLE_BG[r]):"transparent",color:roleFilter===r?(r==="ALL"?"#f5a623":ROLE_C[r]):"#555",fontSize:10,fontWeight:700,cursor:"pointer"}}>
            {r==="ALL"?"Tous":ROLE_L[r]}
          </button>
        ))}
        <span style={{fontSize:10,color:"#555",marginLeft:4,alignSelf:"center"}}>{topCombos.length} combos ≥ ★★★★</span>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(300px,1fr))",gap:12}}>
        {topCombos.map(({key,race,cls,score,reason,tags,role})=>(
          <div key={key} style={{background:"var(--card)",borderRadius:12,padding:16,border:"1px solid "+(SC[score]||"#555")+"25",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,${race.color},${cls.color})`}}/>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              <div>
                <div style={{fontSize:13,fontWeight:800,color:"#fff",marginBottom:6}}>
                  <span style={{color:race.color}}>{race.emoji} {race.name}</span>
            ...
            <p style={{fontSize:11,color:"#888",margin:"0 0 10px",lineHeight:1.5}}>{reason}</p>
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              {tags.map(t=><span key={t} style={{fontSize:8,background:(SC[score]||"#888")+"15",color:SC[score]||"#888",padding:"2px 7px",borderRadius:4,fontWeight:700}}>{t}</span>)}
            </div>
          </div>
        ))}
      </div>
    ...
    {/* ── MATRICE ── */}
    {view==="matrix"&&<div>
      <p style={{fontSize:11,color:"#555",marginBottom:12}}>Clique sur une race ou classe pour filtrer. Survol pour voir les détails du combo.</p>
      <div style={{overflowX:"auto",borderRadius:8,border:"1px solid var(--brd)"}}>
        <table style={{borderCollapse:"collapse",fontSize:9}}>
          <thead>
            <tr style={{background:"#0b1628"}}>
              <th style={{padding:"8px 10px",color:"#555",textAlign:"left",minWidth:90,borderBottom:"1px solid var(--brd)",borderRight:"1px solid var(--brd)",fontWeight:700,fontSize:9}}>Race ↓ \ Classe →</th>
              {CLASSES.map(c=>(
                <th key={c.id} onClick={()=>setHlClass(hlClass===c.id?null:c.id)}
                  style={{padding:"6px 8px",color:hlClass===c.id?c.color:"#555",cursor:"pointer",whiteSpace:"nowrap",background:hlClass===c.id?c.color+"12":"transparent",borderBottom:"1px solid var(--brd)",fontWeight:hlClass===c.id?800:600,textAlign:"center",minWidth:70,transition:"all .1s"}}>
          ...
          <tbody>
            {RACES.map((r,ri)=>(
              <tr key={r.id} style={{background:hlRace===r.id?r.color+"08":ri%2===0?"transparent":"#ffffff02"}}>
                <td onClick={()=>setHlRace(hlRace===r.id?null:r.id)}
                  style={{padding:"6px 10px",color:hlRace===r.id?r.color:"#666",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",borderRight:"1px solid var(--brd)",borderBottom:"1px solid #1a1610",display:"flex",alignItems:"center",gap:5,transition:"all .1s"}}>
                  <span style={{fontSize:13}}>{r.emoji}</span><span>{r.name}</span>
                </td>
                {CLASSES.map(c=>{
                  ...
                  return(
                    <td key={c.id} title={`${r.name} + ${c.name} — ${stars(syn.score)}\n${syn.reason}`}
                      style={{textAlign:"center",padding:"5px 4px",background:hl?sc+"18":"transparent",borderBottom:"1px solid #1a1610",cursor:"default",transition:"background .1s"}}>
                      <div style={{display:"inline-flex",flexDirection:"column",alignItems:"center",gap:1}}>
                        <span style={{fontSize:syn.score>=4?12:9,color:syn.score>=4?sc:"#222",fontWeight:syn.score>=4?900:400,lineHeight:1}}>
      ...
      <div style={{display:"flex",gap:12,marginTop:8,fontSize:9,color:"#555",flexWrap:"wrap"}}>
        {[5,4,3,2,1].map(s=><span key={s} style={{display:"flex",alignItems:"center",gap:4,color:SC[s]||"#555"}}>{"★".repeat(s)} {RATING[s]?.split(" ").slice(1).join(" ")}</span>)}
      </div>
    </div>}
  </div>);
}
...
// ═══════════════════════════════════════════
function ImportDialog({onImport,onClose}){const[code,setCode]=useState("");const[err,setErr]=useState("");const go=()=>{const r=decodeBuild(code.trim());if(r){onImport(r);onClose()}else setErr("Code invalide")};return(<div style={{position:"fixed",inset:0,background:"#000b",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}} onClick={onClose}><div onClick={e=>e.stopPropagation()} style={{background:"#111d33",borderRadius:"var(--radius-md)",padding:24,width:400,maxWidth:"90vw",border:"2px solid #2a2218"}}><h3 style={{margin:"0 0 12px",color:"#fff",fontSize:16}}>📥 Importer un build</h3><textarea value={code} onChange={e=>{setCode(e.target.value);setErr("")}} placeholder="Colle ton code ici..." style={{width:"100%",minHeight:80,background:"#0b1528",border:"1px solid #2a2218",borderRadius:"var(--radius-md)",color:"#ccc",padding:10,fontSize:12,fontFamily:"monospace",resize:"vertical",boxSizing:"border-box"}}/>{err&&<div style={{color:"#ff6b6b",fontSize:11,marginTop:4}}>{err}</div>}<div style={{display:"flex",gap:8,marginTop:12,justifyContent:"flex-end"}}><button onClick={onClose} style={{padding:"8px 16px",borderRadius:8,border:"1px solid #333",background:"transparent",color:"#888",cursor:"pointer",fontWeight:600,fontSize:12}}>Annuler</button><button onClick={go} style={{padding:"8px 16px",borderRadius:8,border:"none",background:"#f5a623",color:"#000",cursor:"pointer",fontWeight:700,fontSize:12}}>Importer</button></div></div></div>)}
...
function ShareCard({ state, onClose }) {
  ...
  const drawCard = () => {
    ...
    // Border
    ctx.strokeStyle = "#2a2218"; ctx.lineWidth = 1.5; roundRect(ctx, 0, 0, W, H, 16); ctx.stroke();
    ...
    // Divider
    ctx.fillStyle = "#2a2218"; ctx.fillRect(24, 68, W - 48, 1);
    ...
    STATS.forEach((stat, i) => {
      ...
      ctx.fillStyle = "#2a2218"; roundRect(ctx, x, y + 18, barW, barH, 2); ctx.fill();
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000c", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ maxWidth: 640, width: "95vw" }}>
        <div style={{ background: "#0b1120", borderRadius: 16, padding: 20, border: "1px solid #2a2218" }}>
...
function BuildCreator({ initialCode, onClearInitialCode, onPublishToCommunity }) {
  ...
  const { activeRace, inn, totalAug: totalAugBonus, postBonus, augMult, augLock, finalStats: finalComputedStats } = computeFullStats({
    race: selectedRace, evoId: selectedEvo || selectedRace?.id,
    c1: primaryClass, t1: primaryTier, c2: secondaryClass, t2: secondaryTier,
